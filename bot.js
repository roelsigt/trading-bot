...
// Europese tickers via Yahoo Finance (suffix nodig)
const EU_TICKERS = [
  // AEX
...
// Financial Datasets voor US tickers
async function getUSData(ticker) {
  try {
    ...
    return calcMetrics(ticker, prices.map(p => ({ close: p.close, volume: p.volume, high: p.high, low: p.low })));
  } catch (err) { return null; }
}

// Yahoo Finance voor Europese tickers
async function getEUData(ticker, naam, markt) {
  try {
    ...
    const metrics = calcMetrics(ticker, combined);
    if (metrics) { metrics.naam = naam; metrics.markt = markt; }
    return metrics;
  } catch (err) { return null; }
}

function calcMetrics(ticker, prices) {
...
<div style="max-width:600px;margin:0 auto;padding:20px;">
  ...
  <div style="background:#FAEEDA;border-radius:8px;padding:10px 14px;margin-bottom:12px;font-size:12px;color:#854F0B;">
    ⏰ Europese markten openen om 09:00 Vlaamse tijd — stel je orders in voor opening.
  </div>
  ${euCards}
...
async function runBot() {
  ...
  for (const data of euCandidates) {
    try {
      ...
      await sleep(1000);
    } catch (err) { console.error(`  ❌ ${data.naam}:`, err.message); }
  }
